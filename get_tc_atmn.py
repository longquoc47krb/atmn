...
class AtmnDriver:
    def __init__(self):
        load_dotenv()
        self.AUTH_TOKEN = os.getenv('ATMN_TOKEN')
    def get_bill_info(self, bill_id):
        ...
        if "Data" in response and "PageData" in response["Data"] and len(response["Data"]["PageData"]) > 0:
            ...
            return ""
    def simplify_and_merge(self, long_text):
        # Tách các cụm bằng dấu phẩy
        items = long_text.split(',')
        
        # Lấy phần tiền tố chung
        common_prefix = items[0].split(' HD ')[0].split(' ')[0] + ' ' + items[0].split(' ')[1]  # "Thu tiền"
        
        # Tách từng phần cụ thể (loại hình và mã HD)
        specific_parts = [item[len(common_prefix):].strip() for item in items]
        
        # Gộp lại thành chuỗi ngắn hơn
        simplified_sentence = f"{common_prefix} {', '.join(specific_parts)}"
        return simplified_sentence
        
    def get_paging_detail(self, refid, bill_id):
        cookies = {
            'cf_clearance': 'wd2f8L.7ie3w2qUzu6rjMwj.8ytKtkxeMqq1mAp0mVw-1735491457-1.2.1.1-KHH9DiP1flNz32OvxN8uWgX.r.dMx9OkRUeEezQmK1exnYDb5ahlPs05TMeTTxhh3jj3OD59tPFjZdNNMD5M54kqBfexGu8swd6Z9rR0aYqyHCi459hWP_Xg0GB4kb19Ufh027vg1WnQeTGPFwvn57nJp2Um8APL8wT6IRk.ZmPnBzbt8UKixpfG.QmDOJN4dGx8Zjs9YpBzil7XU_oHSHUB5WG2YHQRA0X3f_.V3zhyTNoUqqoiOKmu3jOHEjKMwisGvg4ISt0SdC8Z_Px5j2chKP2.rqOPZvKLbBYWFSFR4zuWEi3uTv6RVdL9eg_lnEqSGSleE04Kwk6X1M8U0gpEEUyFe6r0tD1H8ENd7k5E9.F35DxrOb7xOtvas0lz9zNrp2akB1x_5VRRsq.CBg',
        ...
        if "Data" in response and "PageData" in response["Data"] and len(response["Data"]["PageData"]) > 0:
            ...
            if good_code == "KIEM DINH":
                return "Thu tiền kiểm định HD " + bill_id
            if good_code == "HUAN LUYEN":
                return "Thu tiền huấn luyện HD "+ bill_id
            if good_code == "HIEUCHUAN":
                return "Thu tiền hiệu chuẩn HD "+ bill_id
            else:
                return "Thu tiền đào tạo HD "+ bill_id
        else:
            return ""
